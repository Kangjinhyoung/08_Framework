...
public class ParameterController {
	...
	public String paramMain() {
		...
		// classpath:/templates/param/param-main.html로 forward 
...
//	@RequestMapping(value = "test1", method = RequestMethod.GET)
	...
	public String test1(HttpServletRequest req) {
		...
		// 매개 변수 HttpServletRequest req 사용 가능할까?? -> 가능
		// 왜? Spring 의 ArgumentResolver 때문에 가능하다
		// ArgumentResolver(전달 인자 해결사)
		// -> Controller 메서드에 작성된 매개 변수 타입에 맞는 객체(Bean)가 존재하면 바인딩 없으면 생성해서 바인딩 
	  // https://docs.spring.io/spring-framework/reference/web/webflux/controller/ann-methods/arguments.html
		...
		// classpath:/templates/param/result1.html 로 forward
		return "param/result1";
		
		// forward 란??
		// - 클라이언트에 요청에 대한 응답 처리를 대신 해달라고 넘기는 것
	...
	public String test2(
		...
		// -> HTML에서 얻어오는 모든 값은 기본적으로 String 형태이나 Spring 이 
		//    매개 변수에 기입된 타입으로 자동으로 형변환 / 파싱을 수행해준다!!
	...
	public String test4(
		...
		/* type="text" 관련 input 태그는 값을 
		 * 작성하지 않은 경우 빈칸("")이 제출된다!!
		 * -> 제출되는 값이 존재하기 때문에 @RequestParam()에 아무 속성을 적지 않아도 잘 얻어와진다!!
		 * 
		 ...
		 *   --> 만약 제출되지 않을 경우 HTTP 상태코드 400 -> 400 Bad Request 에러 발생
		 *   
		 * * checkbox, radio : 체크되지 않으면 제출 X -> 400 에러
		 * * 직접 쿼리스트링을 작성하는데 key=value 가 누락된 경우 -> 400 에러 
		 * * required = false 일 때 값이 제출되지 않으면 null -> 400 에러 안뜸
		 * 
		 * * defaultValue
		 *   - required = false 이면서, 값이 제출되지 않았을 경우 매개 변수에 대입할 값 지정
		 */
		log.debug("ID : {}", id);
		log.debug("PW : {}", pw);
		log.debug("아이디 저장 : {}", save);

		/* Spring 에서 redirect 하는 방법 */
		// -> 리다이렉트 하려는 요청 주소 앞에 "redirect:" 붙여주면 된다!!
		return "redirect:/param/main"; 
	}
	
	/* 5. @ModelAttribute을 이용한 파라미터 얻어오기 */
	//@ModelAttribute
	// - DTO(또는 VO)와 같이 사용하는 어노테이션
	
	// - 전달 받은 파라미터의 name 속성 값이같이 사용되는 DTO의 
	//   필드명과 같다면 자동으로 setter를 호출해서 필드에 값을 세팅
	...
	public String test5(
			...
			// -> key 값과 변수명이 같은 경우 원래는 @RequestParam의() 생략이 가능했었지만 지금은 생략하면 오류가 발생

...
public class ExampleController {
	...
	// 1) page : 현재 페이지
	// 2) request : 요청 받은 곳 + 요청(forward) 위임 받은 곳 
	// 3) session : 클라이언트가 서버 최초 접속 시 생성, 연결한 브라우저 종료 또는 세션 시간 만료까지 
	// 4) application : 서버 실행 시 1개만 생성, 서버가 종료될 때 까지 유지
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		model.addAttribute("test2", "Model로 세팅한 값ㅋㅋㅋ");
		...
		return "ex/result1"; // classpath:/templates/ex/result1.html 파일로 forward
	}
	 ...
	 * - 해당 어노테이션으로 얻어온 값은 현재 메서드 + forward 한 HTML 파일에서 사용 가능
	 */
	
	@GetMapping("ex3/{number}") // 주소 중 값을 얻어오고 싶은 부분을 {}를 작성하고 {}를 지칭하는 이름(변수명) 내부에 작성
	public String pathVariableTest(
			@PathVariable("number") int num // 주소 중 {number} 자리에 작성된 값을 얻어와 매겨변수 int num에 저장
			) {
		log.debug("num : {}", num);

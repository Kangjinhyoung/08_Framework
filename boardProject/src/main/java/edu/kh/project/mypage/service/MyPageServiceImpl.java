...
@Transactional // 서비스 내 메서드 수행 중 UnCheckedException 발생 시 rollback 수행 아니면 메서드 종료 시 commit 수행
@Service // service 역할 명시 + Bean 등록
@PropertySource("classpath:/config.properties")
public class MyPageServiceImpl implements MyPageService{
	
	@Autowired // 등록된 Bean 중에서 같은 자료형의 Bean 을 의존성 주입(DI)
	...
	private String profileFolderPath; // 이미지 저장 경로
	...
	public int changePw(String currentPw, String newPw, Member loginMember) {
		...
		// 1) 입력 받은 현재 비밀번호가 로그인한 회원의 비밀번호와 일치하는지 검사 

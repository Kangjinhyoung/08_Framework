...
//		if(uploadFile.getSize() <= 0)
		...
		String originalName = fileName.equals("") ? uploadFile.getOriginalFilename() : fileName + ext; //입력된파일명.확장자
	...
	public String test3(MultipartFile uploadFile) {
		...
		try {
			...
			// 2) throws 작성 시 호출하는 메서드에서 추가 예외처리 코드를 작성해야되는 번거로움이 있음
			
			// 3) try-catch 작성 시 메서드 내부에서 예외가 처리되어 메서드 종료 시 
			//    예외가 던져지지 않아 @Transactional이 rollback을 수행할 수 없게 된다!!
			
			// [추천되는 해결 방법]
			// * try-catch를 작성해서 Checked Exception 을 처리
			//   - 호출하는 메서드에 throws 구문 작성 X
			
			// * Unchecked Exception 형태의 사용자 정의 예외 강제 발생
			//   - @Transactional 어노테이션에 rollbackFor 속성 작성 안해도 롤백 처릴 가능!!!
			
			// 예외 강제 발생
			// - Unchecked Exception 은 컴파일러가 자동으로 throws 구문을 작성해줘서 

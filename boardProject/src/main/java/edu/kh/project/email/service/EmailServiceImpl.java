...
// + final 필드 초기화를 위한 생성자 매개 변수에 자동으로 같은 자료형의 Bean 을 의존성 주입함
...
public class EmailServiceImpl implements EmailService{
	...
	// 상수형 필드 선언
	private final JavaMailSender mailSender; // EmailConfig 내용이 적용된 이메일 발송이 가능한 객체(Bean)
	
	private final RedisUtil redisUtil; // Redis(InMemory DB) CRUD 할 수 있는 기능을 제공하는 객체(Bean)
	
	private final SpringTemplateEngine templateEngine; 
	// 자바에서 타임리프를 사용할 수 있게 하는 객체(Bean) // html 코드를 JAVA로 읽어올 수 있음
	...
	public int sendEmail(String htmlName, String email) {
		...
		try {
			...
			// 이메일 발송 시 사용할 html 파일의 이름에 따라 이메일 제목, 내용을 다르게 설정
			switch(htmlName) {
			case "signUp" : emailTitle = "섹시킹진형의 섹시섹시빔 발사."; break;
			case "findPw" : emailTitle = "[boardProject] 비밀번호 찾기 인증번호 입니다."; break;
			}
			...
			// 지정된 HTML 파일에 authKey가 첨부된 후 HTML 코드 전체가 하나의 String 으로 
			// 변환되서 반환 받은 후 변환 된 String 을 메일 내용으로 세팅 
			
			// CID(Content-ID)를 이용해 메일에 이미지 첨부
			helper.addInline("logo", new ClassPathResource("static/images/user.gif"));
			
			// 메일 발송하기
			mailSender.send(mimeMessage);
			
			// Redis 에 이메일, 인증번호 저장(5분 후 만료)
...
/* 추천!!!!!!!!!! */
// 의존성 주입 방법 3 : 생성자를 이용하는 방법(필드 많을 떄)
// private 자료형 필드명1;
// private 자료형 필드명2;

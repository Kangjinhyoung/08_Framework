...
const selectCommentList = () => {
  ...
  .then(response => {
    ...
    // response.ok : http 응답 상태 코드가 200번대(성공)인 경우
  ...
  .then(html => {
    ...
    // 타임리프가 해석된 html 코드를 .comment-list-area의 내용으로 대입 후 HTML 코드를 해석
    commentListArea.innerHTML = html;

    /* [주의 사항] */
    // innerHTML로 새로 만들어진 요소에는 이벤트 리스너가 추가되어 있지 않기 때문에
...
const commentContent = document.querySelector("#commentContent"); // 댓글 내용
...
const deleteComment = (btn) => {
  ...
  .then(result => {
    if(result > 0) { // 성공
      alert("삭제 되었습니다.");
      selectCommentList(); // 댓글 목록 비동기 조회 후 화면 출력
    }
...
const showUpdateComment = (btn) => {
  ...
  cancelBtn.addEventListener("click", () => {
    ...
    // 취소 안함 -> 수정 계속 진행
    if(confirm("취소하겠습니까?") === false) return;

    // 현재 댓글 행 다음 위치에 백업한 원본 댓글 추가
    commentRow.after(beforeCommentRow);
    commentRow.remove(); // 수정 화면으로 변환 된 행 삭제
...
addComment.addEventListener("click", () => {
  ...
  // 1) 로그인 여부 검사 (boardDetail.html의 loginCheck 전역 변수)
  if(loginCheck === false) {
    alert("로그인 후 이용해 주세요.");
    return;
  }
  ...
  // 2) 댓글 작성 여부 검사
  if(commentContent.value.trim().length === 0) { // 내용 X
    alert("내용 작성 후 등록 버튼을 클릭하세요.");
    return;
  }
...
const addEventUpdateComment = () => {
  ...
  btns.forEach(btn => {
    btn.addEventListener("click", () => {
      showUpdateComment(btn); // 수정 버튼 클릭 시 showUpdateComment() 호출
    })
  })
};

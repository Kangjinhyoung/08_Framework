...
changePw?.addEventListener("submit", e => {
  ...
  if(currentPw.value.trim().length == 0) str = "현재 비밀번호 입력하여라";
  if(newPw.value.trim().length == 0) str = "새 비밀번호 입력하여라";
  if(newPwConfirm.value.trim().length == 0) str = "새 비밀번호 확인 입력하여라";

  if(str !== undefined) { // 입력되지 않은 값이 존재한다
    alert(str);
    e.preventDefault(); // form 제출 막기
  ...
  */
  ...
  if( !(lengthCheck && letterCheck && numberCheck && specialCharCheck) ) {
    alert("영어, 숫자, 특수문자 1글자 이상, 6 ~ 20자 사이 입력하여라");
    e.preventDefault();
    return;
  } 
...
secession?.addEventListener("submit", e => {
  ...
  if(memberPw.value.trim().length === 0) { // 입력 안했을 때
    alert("비밀번호 입력해 인마 ~ ㅋ");
    e.preventDefault();
    return;
  }
  ...
  if(agree.checked === false) { // 체크가 되어있지 않은 경우
    alert("탈퇴 할거면 동의 체크해 인마 ~ ㅋ");
    e.preventDefault();
    return;
  }

  // 3) confirm을 이용해서 탈퇴할 건지 확인
  if (confirm("정말 탈퇴 할거야 인마 ~ ㅋ??") === false) { // 취소 클릭 시
    alert("탈퇴 취소ㅋ");
    e.preventDefault();
    return;
  } 
...
// 프로필 이미지 없로드 상태에 따라서 어떤 상태인지 구분하는 값
// -1 : 프로필 이미지를 바꾼적이 없음 (초기상태)
//  0 : 프로필 이미지 삭제(X 버튼 클릭)
//  1 : 새 이미지 선택
let statusCheck = -1;
 ...
 const updatePreview = (file) => {
  ...
  imageInput.addEventListener("change", e => {
    ...
    if(file === undefined) {
      ...
      /* 이전 선택한 파일 유지하는 코드 작성 */
      // -> 이전 선택한 파일을 저장할 전역 변수 선언
      ...
      // DataTransfer가 가지고 있는 files 필드에 lastVaildFile추가

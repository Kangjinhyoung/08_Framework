...
function getCookie(key) {
  ...
  // 3. 쪼개진 배열 요소를 하나씩 꺼내서 JS 객체에 K:V 형태로 추가
...
const selectMemberList = () => {
  ...
  // 1) 비동기로 모든 회원의 회원 번호, 이메일, 탈퇴상태 조회
  fetch("/selectMemberList")
  .then(response => {
    // 응답 성공시 JSON 형태의 응답 데이터를 JS 객체로 변경
  ...
  .then(list => {
    ...
    // 2. 조회 결과인 list를 반복 접근해서 #memberList에 조회된 내용으로 tr, td, th 만들어 넣기
    list.forEach(member => { // 매개 변수 member == 조회된 list에서 하나씩 꺼낸 요소
      ...
      th5.append(initBtn);
       ...
       chnageBtn.innerText = "탈퇴 상태 변경";
       th6.append(chnageBtn);

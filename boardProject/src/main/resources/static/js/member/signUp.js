...
/* 필수 입력 항복의 유효성 검사여부를 체크하기 위한 객체(체크리스트) */
const checkObj = {
  "memberEmail"     : false,
  "memberPw"        : false,
...
emailMessageObj.invaild = "알맞은 이메일 형식으로 작성해 인마 ~ ㅋ";
emailMessageObj.duplication = "이미 사용중인 이메일이야 인마 ~ ㅋ";
...
memberEmail.addEventListener("input", e => {
  ...
  .then(response => {
    if(response.ok) { // HTTP 응답 상태 코드 200번(응답 성공)
      return response.text(); // 응답 결과를 text로 변환 파싱
    }
...
nickMessageObj.normal = "한글, 영어, 숫자로만 3 ~ 10글자 입력";
nickMessageObj.invaild = "닉네임 다시 작성해 인마 ~ ㅋ";
nickMessageObj.duplication = "이미 쓰고 있어 인마 ~ ㅋ";
...
telMessageObj.invaild = "똑바로 써 인마 ~ ㅋ";
telMessageObj.check = "굿";
...
memberTel.addEventListener("input", () => {
  ...
  const regEx = /^010[0-9]{8}$/; // 010시작 이후 숫자 8개
...
const pwMessageObj = {};
pwMessageObj.normal = "영어, 숫자, 특수문자(!,@,#,-,_) 6~20글자 사이로 입력해주세요.";
pwMessageObj.invaild = "똑바로 써 인마 ~ ㅋ";
pwMessageObj.vaild = "잘 썼다 인마 ~ ㅋ";
pwMessageObj.error = "비밀번호 틀렸어 인마 ~ ㅋ";
pwMessageObj.check = "굿";
...
const sendAuthKeyBtn = document.querySelector("#sendAuthKeyBtn"); // 인증 번호 받기 버튼
const authKeyMessage = document.querySelector("#authKeyMessage"); // 인증 관련 메시지 출력 span

const initTime = "05:00"; // 인증 초기 시간 저장
...
/* 전달 받은 숫자가 10 미안(한 자리 수) 인 경우 앞에 0을 붙여서 반환하는 함수 */
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  // 2) 입력된 이메일과 인증 번호를 비동기로 서버에 전달하여 Redis에 저장된 이메일, 인증번호와 일치하는지 확인
  
  /* AJAX로 여러 데이터를 서버로 전달하고 싶을 땐 JSON 형태로 값을 전달해야 한다!! */
...
signUpForm.addEventListener("submit", e => {
  ...
  // 유효성 검사 체크리스트 객체에서 하나씩 꺼내서 false인 경우가 있는지 검사
  for(let key in checkObj) { 
    if( checkObj[key] === false ) { // 유효하지 않은 경우
      ...
      switch(key) {
        ...
        case "authKey"         : str = "이메일이 인증 되지 않았습니다.";     break;
      }
      alert(str); // 경고 출력
  ...
  if(empty < 3 && notEmpty < 3 ) {
    alert("주소가 유효하지 않아요~ㅋ (모두 작성 또는 미작성)");
    e.preventDefault();
    return;
  }
});
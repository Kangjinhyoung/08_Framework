...
1) 클라이언트가 서버에 연결 요청
   -> 클라이언트가 서버로 부터 데이터를 받기 위한 대기 상태에 돌입(EventSource)
...
const connectSse = () => {
  ...
  eventSource.addEventListener("message", e => {
    console.log(e.data); // e.data == 전달 받은 메시지
                         // -> Spring HttpMessageConvert가 Json으로 변환해서 응답해줌
...
/* 알림 메시지 전송 함수 
  ...
  2) 연결된 클라이언트 대기 명단(emitters)에서 클라이언트 id가 일치하는 
     회원을 찾아 메시지 전달하는 send() 메서드 수행

  3) 서버로부터 메시지를 전달 받은 클라이언트의 
     eventSource.addEventListener()가 수행 됨
*/
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* 쿼리스트링 중 cn(댓글 번호)가 존재하는 경우 해당 댓글을 찾아 화면을 스크롤해서 이동하기 */
  ...
  if(cn != null) { // cn이 존재하는 경우
    ...
    window.scrollTo({
      ...
      behavior : "smooth" // 부드릅게 동작(행동)하도록 지정
    });
  }

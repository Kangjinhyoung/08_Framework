...
function getCompleteCount() {
  /* 첫 번째 then
     - 비동기 요청 결과(응답)에 따라 어떤 코드를 수행할 지 제어
     - 매개 변수 response : 응답 결과, HTTP 상태 코드, 요청 주소 등이 담겨 있는 객체

     두 번째 then
     - 비동기 요청으로 얻어온 값을 이용해서 수행 될 JS 코드 작성하는 구문

  */
  fetch("/todo/completeCount") // 비동기 요청해서 결과 데이터 응답 받아오기
  .then(response => {
...
function selectTodoList() {
  ...
  .then(result => {
    ...
    for(let todo of todoList) { // let : 변수 const : 상수
 ...
 * @param url : /todo/detail/10 이런 형태 
 */
...
changeComplete.addEventListener("click", () => {
  ...
  // 팝업 레이어에 작성된 todoNo내용 얻어오기
  const todoNo = document.querySelector("#popupTodoNo").innerText;
  
  // 비동기로 완료 여부 변경
  /* ajax(비동기) 요청 시 사용 가능한 데이터 전달 방식 (REST API와 관련됨)
     GET    : 조회 (SELECT)
     Post   : 삽입 (INSERT)
     PUT    : 수정 (UPDATE)
     DELETE : 삭제 (DELETE)
...
updateView.addEventListener("click", () => {
  ...
  // - dateset 속성 : 요소에 js에서 사용할 값(data)를 추가하는 속성
  //   요소.dataset.속성명 = "값" -> 대입
